const ConfigReviewForm = {
  ...
  ResponseDatabase: {
    ...
    RequesterHeader: 'Họ và tên người đề xuất',
  },
  ApprovalFlowForms: {
...
function submittingReviewForm(data) {
  ...
  const ws = SpreadsheetApp.openById(ConfigReviewForm.ResponseDatabase.WorkSheet)
...
function SubmitReviewForm() {
  ...
  const responseWS = SpreadsheetApp.openById(ConfigReviewForm.ResponseDatabase.WorkSheet);
  ...
  const employeeHeader = ConfigReviewForm.ResponseDatabase.RequesterHeader
  ...
  this.sendCompletedNotification = (taskId) => {
    ...
    template.status = status
    template.approvers = approvers
    template.approvalProgressUrl = `${this.url}?appType=reviewform&responseId=${responseId}`
    ...
    GmailApp.sendEmail(ConfigReviewForm.ApprovalFlowForms.NotificationEmail, subject, "", options)
  }
