const ConfigResignForm = {
  ...
  ResponseDatabase: {
    ...
    RequesterHeader: 'Họ và tên nhân viên yêu cầu',
  },
  ApprovalFlowForms: {
...
function getApprovalFlowsBMHR0202() {
  const sheet = SpreadsheetApp.openById(ConfigResignForm.ApprovalFlow.WorkSheet);
  const data = sheet.getSheetByName(ConfigResignForm.ApprovalFlow.SheetName).getDataRange().getValues();
...
function submittingResignForm(data) {
  ...
  const ws = SpreadsheetApp.openById(ConfigResignForm.ResponseDatabase.WorkSheet)
...
function SubmitResignForm() {
  ...
  const responseWS = SpreadsheetApp.openById(ConfigResignForm.ResponseDatabase.WorkSheet);
  ...
  const employeeHeader = ConfigResignForm.ResponseDatabase.RequesterHeader
  ...
  this.sendCompletedNotification = (taskId) => {
    ...
    template.status = status
    template.approvers = approvers
    template.approvalProgressUrl = `${this.url}?appType=resignform&responseId=${responseId}`
    ...
    GmailApp.sendEmail(ConfigResignForm.ApprovalFlowForms.NotificationEmail, subject, "", options)
  }
  ...
  this.onFormSubmit = () => {
    ...
    if (email == ConfigResignForm.ApprovalFlowForms.AutoApproveEmail) {
